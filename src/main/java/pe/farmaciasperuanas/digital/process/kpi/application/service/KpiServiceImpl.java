...
//                .then(processRoasGeneral())
    ...
    public Mono<Void> processImpressionsParent() {
        return Flux.defer(() -> kpiRepository.generateKpiImpressionsParents())
                .doOnNext(kpi -> System.out.println("Procesando KPI ImpressionsParent: " + kpi))
                .then();
    }

    public Mono<Void> processShippingScopeParent() {
        return Flux.defer(() -> kpiRepository.generateKpiShippingScopeParents())
                .doOnNext(kpi -> System.out.println("Procesando KPI ShippingScopeParent: " + kpi))
                .then();
    }
